...
public class MainActivity extends AppCompatActivity {
    ...
    private void updateTrainingList(TextView txt) {
        ...
        for (CrewMember m : gm.storage.getAllCrew()) {
            sb.append(m.name)
                    .append(" (XP: ").append(m.experience)
                    .append(", Trainings: ").append(m.trainings)
                    .append(")\n");
        }
    ...
    private void showMissionControlView() {
        ...
        btnStart.setOnClickListener(view -> {
            List<CrewMember> crew = gm.storage.getAllCrew();
            if (crew.size() < 2) {
                txtLog.setText("Not enough crew for a mission.\n");
                return;
            }
            currentMission = gm.missionControl.createMission(crew.get(0), crew.get(1));
    ...
    private void showStatisticsView() {
        ...
        for (CrewMember m : gm.storage.getAllCrew()) {
            sb.append(m.name)
                    .append(" | Missions: ").append(m.missionsCompleted)
                    .append(" | Wins: ").append(m.wins)
                    .append(" | Trainings: ").append(m.trainings)
                    .append("\n");
        }
